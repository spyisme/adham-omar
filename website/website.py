...
@website.route("/backend/register", methods=["POST"])
def backend_register():
    try:
        ...
        student_name = data.get("student_name", "").title()
        student_email = data.get("student_email", "").lower()
        student_phone = data.get("student_phone", "")
        parent_phone = data.get("parent_phone", "")
        student_phone_country_code = data.get("student_phone_country_code", "2")
        parent_phone_country_code = data.get("parent_phone_country_code", "2")
        ...
        if existing_user:
            return jsonify({"error": "Email or phone number or parent phone number already exists"}), 409

        profile_picture_filename = "default.png"
        # Handle profile picture if sent as base64 or file
        if "profile_picture" in data and data["profile_picture"]:
            try:
                upload_folder = os.path.join(current_app.root_path, "static", "profile_pictures")
                img_data = base64.b64decode(data["profile_picture"])
                extension = data.get("profile_picture_extension", "png").lower()
                ...
                with open(temp_path, "rb") as f:
                    ...
                    dummy_file = DummyFile(f, temp_filename)
                    profile_picture_filename = compress_image(dummy_file, upload_folder)
                os.remove(temp_path)
            except Exception as e:
                return jsonify({"error": f"Error uploading profile picture: {str(e)}"}), 400
        ...
        send_whatsapp_message("01111251681", "Error on backend register: " + str(e))
